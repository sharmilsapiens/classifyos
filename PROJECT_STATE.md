...
narrator_from_env() returned None and the run shipped SHAP only ("LLM narratives requested but these env vars are unset …
shipping SHAP only" — confirmed in a live job log). The openai package is already a wheel dependency, so it was purely the
missing creds. **Fix:** FastAPI keeps the creds in its .env and, on a databricks submit that requested narratives
(`cfg.explainability.llm_narratives`) AND has the creds, syncs them into a Databricks SECRET SCOPE via the Secrets REST API
(new `api/databricks.py::sync_llm_secrets` → `secrets/scopes/create` with `initial_manage_principal="users"`, `secrets/put`
per key [key == env-var name], `secrets/acls/put` granting READ to the cluster's run-as principal `AIML_RD` — users-manage
does NOT cover a service principal; override via `CLASSIFYOS_DBRICKS_SECRET_READ_PRINCIPAL`; scope name via
`CLASSIFYOS_DBRICKS_SECRET_SCOPE`, default `classifyos-llm`). Only the scope NAME rides in the Job base_parameters (new
`azure_secret_scope`) — the key NEVER appears in run params or logs. Notebook Cell 3 reads each cred via
`dbutils.secrets.get(scope, key)` into os.environ before `ModelRunner.run()` (report-only try/except); narrator_from_env then
finds them. Idempotent + report-only end-to-end: no creds / narratives off / any sync failure → nothing forwarded → SHAP
only, submit never blocked. Identity facts (verified live): `DATABRICKS_TOKEN` is a workspace USER token
(sharmil.basa@sapiens.com) that can list/create scopes; the cluster runs as the `AIML_RD` service principal. LOCAL backend
byte-identical (in-process narrator reads .env directly). **Tests (all Databricks/Secrets REST mocked via
httpx.MockTransport — CI never touches a live workspace): test_api_jobs.py +6** (sync pushes creds + grants READ; absent-creds
no-op; tolerates an existing scope; submit forwards the scope name ONLY when narratives+creds present, with the key NOT in
params; empty when narratives off / creds absent). Affected suites green: jobs 18/18; databricks+llm_explain+run 82/82.
Hallucination-checked the Secrets REST endpoints (scopes/create · put · acls/put) + `dbutils.secrets.get` against the
Databricks docs. **Demo note:** narratives are baked into a run's stored envelope AT RUN TIME, so existing past runs stay
null on reload — after deploy (FastAPI restart + notebook re-import) execute ONE run, then reload THAT run. Docs updated
(databricks_how_it_works new §, databricks_wisdom, api_short_desc, .env.example new CLASSIFYOS_DBRICKS_SECRET_* vars); prompt
archived under prompts/api_phases/. No /run contract change (narrative exists since schema 1.7) → no schema_version bump.
plan_tweak untouched (no deviation).)
**Prior update (2026-07-23 — Databricks Data Profile from real data):** Claude Code (**NEW — Databricks pre-run Data Profile + feature picker now populated from REAL data.**
...
`api/databricks.py::get_table_columns(catalog, schema, table, user_pat)` — the Unity Catalog
*get-a-table* call `GET /api/2.1/unity-catalog/tables/{catalog}.{schema}.{table}` (user PAT), returning
the `columns` array (raises `DatabricksUnavailable`→503 on a columnless table). New endpoint
`GET /api/v1/databricks/table-profile?catalog=&schema=&table=` (`routes/databricks.py`): a
**databricks-backend guard** (503 in local mode — the picker is only offered there, never a silent
fall-through to manual entry), a **simple-SQL-identifier check** on catalog/schema/table (422 — they're
interpolated into the UC REST path; reuses the engine's `_SQL_IDENTIFIER_RE`), then `_profile_from_columns()`
maps each `ColumnInfo.type_name` → the `inspect_file` column groups (numeric: BYTE/SHORT/INT/LONG/FLOAT/
DOUBLE/DECIMAL; datetime: DATE/TIMESTAMP/TIMESTAMP_NTZ; BOOLEAN → binary+categorical; else categorical)
and returns the **same `InspectProfile` shape as `/upload`** + a `delta` `input_source` + a
`db_snapshots/<catalog>_<schema>_<table>.parquet` `server_path` (mirroring `/input-sources/select`).
Row-level stats (`n_rows`/`n_missing`/`sample`/class distribution) are zeroed/omitted — unavailable from
schema-only metadata (computed on the cluster at run time). Missing PAT → 401; unreachable workspace →
503. **(B) Frontend.** New typed client `getTableProfile({catalog,schema,table}, pat)`; `Upload.tsx`
now fetches the profile on table selection and flows it through the **exact same `applyUpload` plumbing**
a CSV upload / DB-select uses — the shared column table + target dropdown, and the Configure page's
feature selector, all populate identically. The manual target/feature text boxes + "Run on Databricks"
button were **removed**, so a Databricks source goes Upload → Configure → Run just like a file; a
"Loading table schema…" spinner covers the fetch; an error is shown with retry. **Tests:** backend
`test_api_databricks.py` +6 (profile shape from a mocked get-a-table, 503-when-not-databricks, no-columns
503, missing-PAT 401, bad-identifier 422, workspace-404 503 — all Databricks REST **mocked** via
`httpx.MockTransport`); frontend `upload.test.tsx` +3 (loading→`applyUpload`, error state, shared column
picker populates from a UC profile). **Full frontend 162 vitest green; `tsc -b` clean; backend Databricks
suite 12/12 green (full suite run separately).** **Hallucination check ✅** — the `ColumnInfo` fields
(`name`/`type_name`/`type_text`/`nullable`/`comment`/`position`) + the `ColumnTypeName` enum verified
against Microsoft Learn (`databricks tables get FULL_NAME`) and the Databricks SDK `catalog.py`. **No
new plan_tweak** — this realizes the deferred #48 follow-up (like the Interim-2b UI realized its deferred
picker), not a deviation. `docs/api_contract.md` (endpoint table + a dedicated section + footer note),
`api_short_desc.md`, `frontend_short_desc.md` updated; generation prompt archived at
`prompts/api_phases/databricks_table_profile_column_picker.md`. **Engine + locked `/run` envelope
UNTOUCHED** (byte-identical). **NOT run on a real cluster yet** (mocked in tests; a live fetch is
exercised once cluster access is available).)
**Prior update (2026-07-14 — Databricks orchestration, §6.6 Step 6):** Claude Code (Databricks orchestration, §6.6 Step 6: FastAPI → Databricks Jobs
...
| 2026-07-14 | **Databricks UC table-profile** (closes plan_tweak #48): new endpoint `GET /api/v1/databricks/table-profile?catalog=&schema=&table=` + client fn `get_table_columns` calling the UC *get-a-table* API (`GET /api/2.1/unity-catalog/tables/{full_name}`, user PAT). Reshapes the `columns` array into the **same `InspectProfile` shape as `/upload`** (`_profile_from_columns` maps `ColumnTypeName` → the inspect column groups) + a `delta` `input_source` + a `.parquet` `server_path` — mirroring `/input-sources/select`. Frontend: `getTableProfile` client + `Upload.tsx` fetches on table-select and flows through the SAME `applyUpload` plumbing (shared column picker + Configure feature selector); the manual target/feature boxes + "Run on Databricks" button removed. Gated on the databricks backend (else 503); identifiers checked (422); missing PAT 401; columnless/unreachable 503. Upload/profile-side — **no `schema_version` bump** (stays 1.11). **Backend `test_api_databricks.py` +6; frontend `upload.test.tsx` +3 → 162 vitest · tsc clean.** | Realizes the UC-profiling follow-up deferred in Step 6 (proxies returned names only), exactly like the Interim-2b UI realized its deferred picker — **not a deviation**, so no new plan_tweak (it closes #48). **Reuse-not-branch**: returning the `/upload` `InspectProfile` shape + a `delta` `input_source` lets the frontend treat a UC table identically to a file/DB source through the existing `applyUpload` — no parallel UI. Row-level stats (`n_rows`/`n_missing`/`sample`/class dist.) are zeroed/omitted because schema-only metadata has no data — honest, not fabricated (the cluster computes them at run time). Backend-gated 503 (never a silent fall-through to manual entry) mirrors the rest of the orchestration layer. Identifier check guards the UC REST path (interpolated `catalog.schema.table`), reusing the engine's `_SQL_IDENTIFIER_RE`. Hallucination check ✅ — `ColumnInfo`/`ColumnTypeName` verified against Microsoft Learn (`databricks tables get`) + the Databricks SDK `catalog.py`. All Databricks REST **mocked** in tests (no live cluster). NOT run on a real cluster yet |
...
| 2026-07-14 | Databricks UC table-profile — populate the column picker from a table's schema (closes plan_tweak #48) | New `GET /api/v1/databricks/table-profile` (client `get_table_columns` → UC *get-a-table*) reshaping the `columns` array into the **same `InspectProfile` shape as `/upload`** + a `delta` `input_source` + a `.parquet` `server_path`; frontend `getTableProfile` + `Upload.tsx` flows it through the SAME `applyUpload` plumbing (shared column picker; Configure feature selector), removing the manual target/feature boxes + "Run on Databricks" button. Backend-gated (503 in local mode), identifier-guarded (422), 401/503 error paths. Upload/profile-side — **no `schema_version` bump** (stays 1.11). Backend `test_api_databricks.py` +6; frontend `upload.test.tsx` +3 → **162 vitest · tsc clean**. Hallucination check ✅ (`ColumnInfo`/`ColumnTypeName` vs Microsoft Learn + databricks-sdk `catalog.py`). All Databricks REST mocked; NOT run on a real cluster. Prompt archived to `prompts/api_phases/databricks_table_profile_column_picker.md` |
| 2026-07-23 | Databricks — moved the `/run` envelope reshaper + response models into the engine wheel (`classifyos.envelope`) so the Job notebook needs no repo checkout | Fixes a real cluster failure (`No module named 'api'` — the Databricks Git folder was a partial/sparse checkout without `backend/`). New `classifyos/envelope/` (`result_builder` + `serialize` + `artifacts` + `schema` (response pydantic models + `SCHEMA_VERSION`) + one-call `build_run_envelope`); `api.result_builder`/`serialize`/`artifacts` become re-export shims and `api.models` re-imports the response models (so `api.models.RunResponse` **IS** `classifyos.envelope.schema.RunResponse` — one definition; every existing api/test import unchanged). Notebook Cell 5 → `classifyos.envelope.build_run_envelope` from the wheel; Cell 3 `api`-bootstrap deleted. **`pydantic>=2.6,<3` added to engine wheel deps** (owner-approved; data-lib not a web *server* dep, already transitive via mlflow; a pure-engine CLI run never imports envelope) — plan_tweak #50. **NO `/run` contract / `schema_version` change** (byte-identical: both paths use the same `RunResponse.model_dump(by_alias=True)`). **Requires wheel rebuild + re-upload.** Also fixed notebook MLflow experiment → absolute `/Users/<user>/…` path (was non-fatal) + corrected `databricks_how_it_works.md` path-matching (`get_task_run_id`). Backend suite green (75 API + full); local backend byte-identical. NOT run on a real cluster (pending wheel re-upload + notebook pull) |
| 2026-07-23 | Per-user Runs from Databricks MLflow (read-path) | The Runs tab now lists/reloads the CALLER'S OWN runs from Databricks-managed MLflow (was: all runs from the local store). **Engine:** shared snapshot constants + `USER_EMAIL_TAG` + `snapshot_envelope(run_id, envelope, user_email)` in `classifyos.mlflow_logging` (single source; `api.mlflow_read.snapshot_result` delegates); notebook Cell 5 tags each run's owner + logs the reloadable snapshot. **API:** `list_runs(user_email)` server-side tag filter, `load_run(user_email)` ownership-404, `routes/runs.py` resolves the caller's email from the PAT (SCIM) in the databricks backend (missing/expired → 401); local unchanged. **Frontend:** `listRuns`/`loadRun` send the PAT, Runs page prompts for it on Databricks. **Thread-safe** (service-token read; PAT only for identity — no per-request env swap). No contract/`schema_version` change. **Deploy:** FastAPI needs `MLFLOW_TRACKING_URI=databricks` + host/token (see `.env.example`). Hallucination-checked vs mlflow 3.14.0 (`search_runs(filter_string)` + backticked tag key). Tests: `test_api_runs.py` +6, `runs.test.tsx` +2 (all MLflow/Databricks mocked) — full backend suite + 167 frontend green; `tsc -b` clean. plan_tweak #51. Prompt `prompts/api_phases/per_user_runs_from_databricks.md`. NOT run on a real cluster |
| | | |
