...
| `GET`  | `/api/v1/databricks/clusters` | **(1.11, additive)** List the clusters a run can be submitted to → `{clusters: [{cluster_id, cluster_name, state}]}` — only submittable clusters (`state` ∈ `RUNNING`\|`TERMINATED`), sorted by `cluster_name`. Uses the **service token** (not the user's PAT — the service identity submits the Job and picks the cluster), so **no `X-Databricks-Token` is required**. Powers the run-config cluster picker; the chosen `cluster_id` is echoed back on `/run` (optional field). `500` server unconfigured (no host/service token), `503` unreachable. |
...
| `POST` | `/api/v1/runs/{run_id}/narrate` | **(additive, no `schema_version`)** Generate the LLM reason-code narratives (`result.explanations[model].rows[].narrative`, schema 1.7) for a stored run **OFF-CLUSTER** and return the narrated `/run` envelope. Needed because on the **databricks** backend the run executes on a cluster that cannot reach the Azure OpenAI private endpoint (403); the engine ships SHAP + a `narration_context.json` side artifact and FastAPI (which can reach Azure) narrates from it, then RE-persists the narrated snapshot so a reload is instant. **Report-only**: absent creds / absent context / any failure returns the envelope UNCHANGED (`200`), never a `500`. `404` unknown run / no snapshot; `503` store unreachable; `401` missing PAT (databricks). See below. |
...
### `POST /api/v1/runs/{run_id}/narrate` — narrate a stored run off-cluster (additive, no `schema_version`)

Generates the LLM reason-code narratives (`result.explanations[model].rows[].narrative`, schema 1.7)
for a stored run and returns the **same locked `/run` envelope** with the narratives filled in.

**Why it exists.** Narratives are normally generated by the engine DURING a run. On the **databricks**
backend the run executes on a cluster locked to a private endpoint that **cannot reach Azure OpenAI**
(`403 Public access is disabled`), so the engine sets `CLASSIFYOS_NARRATE_IN_ENGINE=false`, ships SHAP
only, and writes a **side artifact** `api/narration_context.json` (the analyst dataset/column context +
the data-derived schema + sample rows — the fields not already in the envelope). FastAPI **can** reach
Azure, so this endpoint rebuilds the full narrator context (that artifact + `class_distribution`,
per-model metrics, global importances and the SHAP rows straight from the run's envelope), narrates
each row, attaches the `narrative`, and **RE-persists** the narrated envelope as the run's snapshot so
a later reload shows the narratives instantly. `narration_context.json` is a side artifact — **not**
part of the locked `/run` envelope — so this is purely additive with **no `schema_version` bump**
(`narrative` has existed since 1.7).

- **Report-only** — absent `AZURE_OPEN_AI_*` credentials, an absent context artifact, an unsupported
  problem type (multilabel), or ANY failure returns the envelope **unchanged** with `200` (never `500`).
- `404` — unknown `run_id`, or a run with no persisted snapshot. `503` — the tracking store is
  unreachable. `401` — missing/expired PAT on the databricks backend (same per-user scoping as reload).
- No new ML — it reuses the engine narrator (`classifyos.analysis.llm_explain`) entirely. The
  Explainability page calls it automatically for a databricks-backed run whose rows lack narratives.
...
// sampled path (a SQL warehouse was reachable) — the full /upload InspectProfile over real UC data:
{
  ...
  "input_source": {                             // makes the run read the Delta table (§6.6 Step 4)
    "type": "delta",
    "connection_env": "CLASSIFYOS_PG_DSN",      // unused for delta; kept for the InputSourceConfig shape
    "catalog": "main",
    "schema": "insurance",
    "table": "policy_lapse",
    "query": null
...
_2026-07-14 (additive, **no schema/version change** — rides the upload/profile side, not the locked
`/run` envelope): added `GET /api/v1/databricks/table-profile` — fetch a Unity Catalog table's schema
and return it in the same `InspectProfile` shape as `/upload` (+ a `delta` `input_source` + snapshot
...
_2026-07-24 (additive, **no schema/version change** — `narrative` has existed since 1.7): added
`POST /api/v1/runs/{run_id}/narrate`, which generates the LLM reason-code narratives for a stored run
**off-cluster** and re-persists the narrated snapshot. Moves narration out of the engine run on the
databricks backend (the cluster can't reach the Azure OpenAI private endpoint — 403); the engine gate
`CLASSIFYOS_NARRATE_IN_ENGINE` (default `true` → local unchanged) writes a `narration_context.json`
side artifact instead. Report-only (absent creds/context/failure → envelope unchanged, never 500). The
Explainability page auto-calls it for a databricks-backed run whose rows lack narratives. The previous
Databricks secret-scope handoff for LLM creds is now moot and was removed. See plan_tweak #52 and the
`POST /api/v1/runs/{run_id}/narrate` section above._
