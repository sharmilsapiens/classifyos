...
## 15. LLM reason-code narratives on Databricks (Azure OpenAI via a secret scope)

The opt-in LLM narratives (schema 1.7 — `result.explanations[model].rows[].narrative`) are generated by
the engine **during the run** (`classifyos.analysis.llm_explain.narrator_from_env`, which reads the five
`AZURE_OPEN_AI_*` creds from `os.environ`). On Databricks the run executes on the **cluster**, whose env
has none of those creds — so `narrator_from_env()` returns `None`, logs `"LLM narratives requested but
these env vars are unset … shipping SHAP only"`, and the run ships SHAP only. (Locally the FastAPI
process has them in `.env`, so it works. The `openai` package is already a wheel dependency — it was
purely the missing credentials.)

**Fix — creds ride a secret scope, never the Job params.** The Azure key must not appear in the Job's
run parameters (visible in the Databricks UI), so it is routed through a Databricks **secret scope**:

1. **FastAPI (databricks submit branch, `routes/run.py`):** when the run requested narratives
   (`cfg.explainability.llm_narratives`) AND this host has the creds, call
   `api.databricks.sync_llm_secrets()` (service token) → `secrets/scopes/create`
   (`initial_manage_principal="users"`), `secrets/put` per cred (secret **key == env-var name**), and
   `secrets/acls/put` granting **READ** to the cluster's run-as principal (`AIML_RD` by default;
   `users`-manage does NOT cover a service principal). It returns the scope name (or `None` on absent
   creds / any failure — report-only, never blocks the submit).
2. **Submit:** only the scope **name** rides in the Job `base_parameters` as `azure_secret_scope`
   (non-secret). The key never travels.
3. **Notebook (Cell 3):** if `azure_secret_scope` is set, read each cred via
   `dbutils.secrets.get(scope, key)` into `os.environ` **before** `ModelRunner.run()` (report-only
   try/except). Databricks redacts secret values in notebook output. `narrator_from_env()` then finds
   them and narratives are generated + logged into the run's envelope snapshot.

**Config knobs (FastAPI env, both optional):** `CLASSIFYOS_DBRICKS_SECRET_SCOPE` (default
`classifyos-llm`), `CLASSIFYOS_DBRICKS_SECRET_READ_PRINCIPAL` (default `AIML_RD` — set to the exact
group/SP if that's not the cluster's identity). Requires the service identity (here a workspace **user**
token, `sharmil.basa@sapiens.com`, verified able to list/create scopes) to create the scope + set its
ACL; the cluster runs as the `AIML_RD` service principal (verified via `current_user()`).

**Gating / safety:** opt-in + report-only end-to-end — no creds, narratives off, or any sync failure →
nothing forwarded → SHAP only, submit never blocked. **LOCAL backend unchanged** (in-process narrator
reads `.env`). Additive: the `/run` envelope already carries `narrative` (since 1.7), so **no
`schema_version` bump**. Deploy = FastAPI restart + notebook re-import (no wheel/cluster change).

**Demo note:** narratives are baked into a run's stored envelope **at run time**, so existing past runs
stay `null` on reload — after deploying, run ONCE, then reload THAT run in the Runs tab.
