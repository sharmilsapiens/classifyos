...
## 15. LLM reason-code narratives on Databricks — generated OFF the cluster, in FastAPI

The opt-in LLM narratives (schema 1.7 — `result.explanations[model].rows[].narrative`) were originally
generated by the engine **during the run** (`classifyos.analysis.llm_explain.narrator_from_env`, reading
the five `AZURE_OPEN_AI_*` creds from `os.environ`). On Databricks the run executes on the **cluster**,
and the real blocker turned out **not** to be missing credentials but **network reachability**: the
cluster is locked to a **private endpoint** and cannot reach the public Azure OpenAI endpoint, so every
call fails with `openai.PermissionDeniedError: 403 – Public access is disabled. Please configure private
endpoint.` (confirmed in a live job log). FastAPI **can** reach the endpoint (that is why local works).

> **Superseded approach.** An earlier fix synced the creds into a Databricks **secret scope** so the
> cluster could narrate. That correctly delivered the creds — but the cluster still can't *reach* Azure,
> so it was moot. It was removed (`sync_llm_secrets` + the Secrets REST helpers, the `azure_secret_scope`
> Job parameter, and the notebook's `dbutils.secrets.get` loop). No Azure creds are pushed to the
> workspace anymore.

**Fix — move narration to a FastAPI step.** Narration is split out of the run and done off-cluster:

1. **Engine (wheel change; additive + flag-gated).** The run still computes SHAP. A new flag
   `CLASSIFYOS_NARRATE_IN_ENGINE` (default **true** → local byte-identical) gates the in-process narrate
   call. When a run requests narratives, the engine also serializes a **side artifact**
   `api/narration_context.json` (`ModelRunner._build_narration_context`; logged by
   `mlflow_logging.log_run` next to the result snapshot) carrying the fields the API can't reconstruct
   from the result: the analyst `dataset_context`/`column_context` + `context_mode`, and the data-derived
   per-column schema + a couple of sample rows (+ the feature-column list). It is a SIDE artifact, not the
   locked `/run` envelope → **no `schema_version` bump** (`narrative` has existed since 1.7).
2. **Notebook (Cell 3).** Sets `os.environ["CLASSIFYOS_NARRATE_IN_ENGINE"] = "false"` so the cluster
   skips the unreachable Azure call and just writes SHAP + `narration_context.json`. Also disables MLflow
   OpenAI autologging (`mlflow.openai.autolog(disable=True)`) — no OpenAI calls happen on the cluster now,
   and it was the source of the noisy `"experiment_id is missing"` warnings + latency.
3. **FastAPI — `POST /api/v1/runs/{run_id}/narrate`.** Loads the run's persisted `/run` envelope
   (`api.mlflow_read.load_run`) + `api/narration_context.json` (`load_narration_context`), rebuilds the
   **full** `RunContext` (the artifact + `class_distribution` → base rates, `result.models` → per-model
   metrics, `result.permutation_importance`/`feature_impact` → global features, `result.explanations` →
   SHAP rows), builds the narrator via `narrator_from_env()` (FastAPI's `AZURE_OPEN_AI_*` creds), narrates
   every row via the engine's `narrate_rows` (**no new ML** — `api.narrate` reuses `llm_explain`
   entirely), attaches `narrative`, and **RE-persists** the narrated envelope as the run's snapshot —
   routed to Databricks-managed MLflow with `tracking_uri="databricks"` (mirroring the §6.1 read fix), so
   `load_run` reloads it and a past run shows narratives instantly.
4. **Frontend.** The Explainability page auto-calls `/narrate` for a databricks-backed run (its
   `result.mlflow.tracking_uri` starts with `"databricks"`) whose SHAP rows lack a `narrative`, then swaps
   in the narrated envelope. For the demo the run is pre-narrated once (persisted), so the reloaded past
   run is instant.

**Gating / safety.** Report-only end-to-end: absent creds (`narrator_from_env` → `None`), an absent
context artifact, an unsupported problem type (multilabel), or ANY failure returns the envelope
**unchanged** (`200`, never `500`). Auth mirrors reload — `404` unknown run / no snapshot, `503` store
unreachable, `401` missing PAT on the databricks backend. **LOCAL backend unchanged** (in-process narrator
reads `.env`; the extra context artifact is harmless).

**Deploy footprint.** This DOES require a **wheel rebuild + cluster restart + notebook re-import**
(B-full), since the engine flag + side artifact ship in the wheel and the notebook changed. The
`[RISK]` on the re-persist: the FastAPI **service token** writes `api/run_response.json` to a run created
on the cluster — it needs EDIT on the ClassifyOS experiment; if it can't write, narration still returns
live (report-only) but the instant-reload persistence is skipped.

**Demo note:** narratives are baked into a run's stored envelope **at narrate time**, so existing past
runs stay `null` until re-narrated — after deploying, open a databricks run's Explainability tab once
(auto-narrates + persists), then reload THAT run in the Runs tab.
