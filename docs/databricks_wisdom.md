...
| Explainability shows **no LLM narrative** on Databricks (`LLM narratives requested but these env vars are unset ... shipping SHAP only` in the job driver log) | The `AZURE_OPEN_AI_*` creds live only in the FastAPI `.env`; the narrator runs in the ENGINE on the CLUSTER, which doesn't have them | Set the five `AZURE_OPEN_AI_*` vars on the **cluster** (Compute -> Edit -> Advanced -> Environment variables; key ideally a `{{secrets/<scope>/<key>}}` ref) + restart, then re-run. Narratives are baked into the run snapshot **at run time** — runs from before the fix stay `narrative:null` (reload does NOT backfill), so a fresh run is required |
