...
| LLM **narratives** null on Databricks; job log says `LLM narratives requested but these env vars are unset … shipping SHAP only` | Narratives are generated on the CLUSTER at run time, and the cluster env has no `AZURE_OPEN_AI_*` | **RESOLVED (2026-07-23)** — FastAPI syncs the creds into a Databricks **secret scope** on submit and the notebook reads them (`dbutils.secrets.get`) — the key never rides in Job params. Set `CLASSIFYOS_DBRICKS_SECRET_SCOPE` / `CLASSIFYOS_DBRICKS_SECRET_READ_PRINCIPAL` if the defaults (`classifyos-llm` / `AIML_RD`) don't fit. Existing past runs stay null (re-run after deploy). See `databricks_how_it_works.md` §15 |
