"""JSON-safety helpers for the run-result envelope.
...
two stay consistent — this adds only the NaN/Inf → None step on top.

Lives in the engine (``classifyos.envelope``) so the Databricks Job notebook can build the
envelope from the wheel alone; ``api.serialize`` re-exports :func:`safe_jsonify` unchanged.
"""
