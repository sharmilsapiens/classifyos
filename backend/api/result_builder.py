...
It was extracted verbatim from ``routes/run.py`` (schema 1.0–1.9 logic unchanged) so there is
exactly ONE implementation of the reshaping, reusable by:

* the synchronous ``POST /api/v1/run`` route (local execution backend), and
* the Databricks Job entrypoint notebook (``notebooks/classifyos_job_runner.py``), which runs the
  engine on the cluster and writes this same envelope to the UC output volume for
  ``GET /api/v1/run/{job_id}/results`` to fetch (Databricks execution backend).

Keeping it standalone (no FastAPI import) is what lets the cluster-side notebook reuse it.
"""
