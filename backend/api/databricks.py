...
def _submit_payload(
    ...
    task: dict[str, Any] = {
        ...
        "notebook_task": {
            ...
            # run's parameters in the Databricks UI — a secret-scope handoff is a follow-up. LLM
            # narratives are NOT generated on the cluster (it can't reach Azure), so no Azure creds /
            # secret-scope name ride here; narration happens off-cluster in FastAPI.
            "base_parameters": {
...
# NOTE: an earlier change synced the Azure OpenAI LLM-narrative creds into a Databricks secret scope
# so the CLUSTER could narrate during the run. That is now MOOT — the cluster cannot reach the Azure
# private endpoint (403), so narration was moved OFF the cluster to FastAPI
# (POST /api/v1/runs/{run_id}/narrate; see api.narrate). The secret-scope sync + its Secrets REST
# helpers were removed; no Azure creds are pushed to the workspace anymore.
