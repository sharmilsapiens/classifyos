...
#: Secrets 2.0 — push the Azure OpenAI LLM-narrative creds into a scope so the Job's cluster can read
#: them (``dbutils.secrets.get``) WITHOUT the key ever riding in the Job's run parameters. Verified
#: against the Databricks Secrets REST API (create scope · put secret · put ACL).
_SECRETS_SCOPE_CREATE_PATH = "/api/2.0/secrets/scopes/create"
_SECRETS_PUT_PATH = "/api/2.0/secrets/put"
_SECRETS_ACL_PUT_PATH = "/api/2.0/secrets/acls/put"
...
def _submit_payload(
    ...
    azure_secret_scope: str | None = None,
) -> dict[str, Any]:
    ...
    task: dict[str, Any] = {
        ...
        "notebook_task": {
            ...
            # base_parameters reach the notebook as string widgets. [RISK] the PAT is visible in the
            # run's parameters in the Databricks UI — a secret-scope handoff is the follow-up. NOTE the
            # Azure LLM key is deliberately NOT here: ``azure_secret_scope`` is only the scope NAME
            # (non-secret); the creds live in a Databricks secret scope (see ``sync_llm_secrets``) and
            # the notebook reads them via ``dbutils.secrets.get`` — a model for the PAT hardening too.
            "base_parameters": {
                "run_config": json.dumps(run_config),
                "user_token": user_pat,
                "wheel_path": wheel_path,
                "user_email": (user_email or "").strip() or "unknown_user",
                "azure_secret_scope": (azure_secret_scope or "").strip(),
            },
        },
    }
    if wheel_path:
...
def submit_run(
    ...
    missing config.
    """
    payload = _submit_payload(run_config, user_pat, cluster_id, user_email, azure_secret_scope)
...
# --------------------------------------------------------------------------- #
# Secrets — push Azure OpenAI LLM-narrative creds to a scope (never Job params) #
# --------------------------------------------------------------------------- #
#
# LLM reason-code narratives (Azure OpenAI, schema 1.7) are generated by the engine DURING the run —
# on Databricks that means ON THE CLUSTER, whose environment has none of the ``AZURE_OPEN_AI_*``
# creds, so ``narrator_from_env()`` returns ``None`` and the run ships SHAP only. We bridge that by
# pushing the creds from THIS process's env into a Databricks SECRET SCOPE (service token) and
# forwarding only the scope NAME to the Job; the notebook reads them via ``dbutils.secrets.get``. The
# key therefore never appears in the Job's run parameters or logs (Databricks redacts secret values).
# Report-only throughout: any failure just leaves narratives off — it never blocks a submit.


def llm_secret_scope() -> str:
    """The Databricks secret-scope name the Azure LLM creds are synced into (env-overridable)."""
    return (os.environ.get("CLASSIFYOS_DBRICKS_SECRET_SCOPE") or "").strip() or "classifyos-llm"


def _llm_secret_read_principal() -> str:
    """The principal granted READ on the scope — the cluster's run-as identity.

    ``initial_manage_principal="users"`` (set on scope-create) covers human users but NOT a service
    principal, and this workspace's cluster runs as the ``AIML_RD`` service principal, so the scope
    needs an explicit READ ACL for it. Overridable via ``CLASSIFYOS_DBRICKS_SECRET_READ_PRINCIPAL``
    (set it to the exact group name / SP application-id if ``AIML_RD`` is not the right principal).
    """
    return (os.environ.get("CLASSIFYOS_DBRICKS_SECRET_READ_PRINCIPAL") or "").strip() or "AIML_RD"


def _azure_llm_creds() -> dict[str, str]:
    """The ``AZURE_OPEN_AI_*`` creds present in THIS process's env, keyed by env-var NAME.

    Returns ``{}`` unless the four REQUIRED vars are all present — mirroring
    :func:`classifyos.analysis.llm_explain.narrator_from_env`'s own requirement (imported here as the
    single source of the env-var names), so what we sync is exactly what the cluster narrator needs.
    ``AZURE_OPEN_AI_MODEL`` is optional and included only when set.
    """
    from classifyos.analysis.llm_explain import (  # noqa: PLC0415 — lazy; single source of the names
        ENV_API_KEY,
        ENV_API_VERSION,
        ENV_DEPLOYMENT,
        ENV_ENDPOINT,
    ...
    required = (ENV_ENDPOINT, ENV_API_KEY, ENV_API_VERSION, ENV_DEPLOYMENT)
    values = {name: (os.environ.get(name) or "").strip() for name in (*required, ENV_MODEL)}
    if any(not values[name] for name in required):
        return {}
    return {name: value for name, value in values.items() if value}


def _create_secret_scope(client: httpx.Client, scope: str) -> None:
    """Create ``scope`` (idempotent) with ``initial_manage_principal="users"``.

    A create against an already-existing scope returns ``RESOURCE_ALREADY_EXISTS`` (HTTP 400) — that
    is success for our idempotent sync, so it is swallowed. Any other error propagates.
    """
    try:
        _request(
            client,
            "POST",
            _SECRETS_SCOPE_CREATE_PATH,
        ...
        if "RESOURCE_ALREADY_EXISTS" not in str(exc):
            raise  # a genuine failure (not "already exists") is real


def sync_llm_secrets() -> str | None:
    """Sync the Azure OpenAI LLM creds from this process's env into a Databricks secret scope.

    Returns the scope NAME on success (the caller forwards it to the Job as a base_parameter), or
    ``None`` when the creds are absent or the sync failed — in which case no scope is forwarded and
    the run ships SHAP only. Report-only: NEVER raises, so it can never block a submit. Authenticated
    with the SERVICE token. Idempotent: creates the scope (tolerating "already exists"), overwrites
    the secrets (key == env-var name, so the notebook maps them straight back), and grants the
    cluster identity READ — so repeated submits are safe. The Azure key never rides in Job params.
    """
    creds = _azure_llm_creds()
    if not creds:
        return None  # required creds not configured on this host → nothing to sync
    scope = llm_secret_scope()
    principal = _llm_secret_read_principal()
    try:
        with _build_client(_service_token()) as client:
            _create_secret_scope(client, scope)
            ...
            # Grant the cluster's run-as identity READ (users-manage doesn't cover a service principal).
            _request(
                client,
                "POST",
                _SECRETS_ACL_PUT_PATH,
                json={"scope": scope, "principal": principal, "permission": "READ"},
            )
        return scope
    except Exception:  # noqa: BLE001 — report-only; a sync failure must never block a run submit
        logger.warning(
            "could not sync Azure LLM creds to secret scope %r; run will ship SHAP only", scope
        )
        return None
